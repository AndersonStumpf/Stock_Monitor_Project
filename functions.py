...
from tvdatafeed_lib import TvDatafeed
import unicodedata
...
df_ibov = pd.read_csv('tabela_ibov.csv', encoding='utf-8', sep=';')
df_ibov['Setor'] = df_ibov['Setor'].apply(normalize_text)


#trata os valores de cada coluna do df_ibov
df_ibov['Part (%)'] = pd.to_numeric(df_ibov['Part (%)'].str.replace(',','.'))
df_ibov['Qtde'] = pd.to_numeric(df_ibov['Qtde'].str.replace('.', '').str.replace(',', ''))
df_ibov['Participação'] = df_ibov['Qtde'] / df_ibov['Qtde'].sum()
